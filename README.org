...
Notably useful functions include: CL-DS.ALG:DISTINCT, CL-DS.ALG:IN-BATCHES, CL-DS.ALG:PARTITION-IF, CL-DS.ALG:ARRAY-ELEMENTWISE, CL-DS.ALG:CUMULATIVE-ACCUMULATE.
...
If you would want to generate ID for each person and quarter you would had to use hash-table to keep id-seeds... But why not simply use AGGREGATE again?
...
(serapeum:~>
 ...
 (vellum:transform
  (vellum:bind-row (person month output id)
    (vellum:group-by person (quarter (1+ (truncate (1- month) 4))))
    (vellum:aggregate id (cl-ds.math:sum 1))
    (setf id (vellum:aggregated-value id)))
...
An ever so slightly more convinient way to achieve the goal dare I say.
